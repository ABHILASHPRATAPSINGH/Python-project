...
from src.commands.threadCommands.ExtractPdf2ImagedPdf_Thread import ExtractPdf2ImagedPdf_Thread
...
from src.userform.Pdf2ImagedPdfFrame_WithoutTitle import Pdf2ImagedPdfFrame_WithoutTitle
from src.userform.ext.FilePathSelectionWidgetEx import FilePathSelectionWidgetEx

class Pdf2ImagedPdfFrame_WithoutTitleEx(Pdf2ImagedPdfFrame_WithoutTitle):

    def __init__(self):
        super(Pdf2ImagedPdfFrame_WithoutTitleEx, self).__init__()
        ...
        self._btnExportPdf2Png.clicked.connect(self.exportPdf2ImagedPdf)
    ...
    def exportPdf2ImagedPdf(self):
        ...
        try:
            ...
            self.pdf2ImagedPdfThread=ExtractPdf2ImagedPdf_Thread(pdfFilePath,requiredPageLists)
            self.pdf2ImagedPdfThread.startedSignal.connect(self.threadStarted)
            self.pdf2ImagedPdfThread.processingStatusSignal.connect(self.lblStatusUpdate)
            self.pdf2ImagedPdfThread.maxCurrentPageProgressBarSignal.connect(self.updateProgressBar)
            self.pdf2ImagedPdfThread.completedSignal.connect(self.complete)
            self.pdf2ImagedPdfThread.finallyStatusSignal.connect(self.finalFunc)
            self.pdf2ImagedPdfThread.errorSignal.connect(self.errorFoundFound)
            self.pdf2ImagedPdfThread.start()
...
if __name__ == '__main__':
    ...
    f=Pdf2ImagedPdfFrame_WithoutTitleEx()
    f.show()
    app.exec_()
